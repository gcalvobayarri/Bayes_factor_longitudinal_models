...
source("./simulation_study1/functions/fixed_effects_model/power_full_conditionals/beta0_power_posterior_fixed_effects_model.R")
source("./simulation_study1/functions/fixed_effects_model/power_full_conditionals/sigma_power_posterior_fixed_effects_model.R")


source("./simulation_study1/functions/fixed_effects_model/log_likelihood_fixed_effects_model.R")
...
for (h in 1 : 10) {
  ...
  for(nt in 0 : n.t){
    ...
    for(i in 2 : niter){
      ...
      # beta0
      beta0[nt * niter + i] <- beta0_power_posterior_fixed_effects_model(y, sigma[ nt * niter + i - 1], A, tau_vector[nt + 1])
      
      # sigma
      sigma[nt * niter + i] <- sigma_power_posterior_uniform_prior_fixed_effects_model(y, sigma[nt * niter + i - 1], beta0[nt * niter + i], a, b, tau_vector[nt + 1])
  ...
  for(nt in 0 : n.t){
    for(i in 1 : niter_powpos){
      ...
      log_likelihood[ nt * niter_powpos + i] <- log_likelihood_fixed_effects_model(y, beta0[nt * niter + nburning + i], 
...
app_evidence_power_posterior_fixed_effects_model_simulated_data <- app_evidence_power_posterior

save(app_evidence_power_posterior_fixed_effects_model_simulated_data, 
     file = "./simulation study/results/app_evidence_power_posterior_fixed_effects_model_simulated_data.RData")
