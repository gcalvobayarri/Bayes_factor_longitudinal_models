# Power posterior random trend model

# 0. functions-------------------

source("./simulation_study1/functions/b1_model/power_full_conditionals/b1_power_posterior_random_trend_model.R")
source("./simulation_study1/functions/b1_model/power_full_conditionals/beta0_power_posterior_random_trend_model.R")
source("./simulation_study1/functions/b1_model/power_full_conditionals/sigma_power_posterior_uniform_prior_random_trend_model.R")
source("./simulation_study1/functions/b1_model/power_full_conditionals/sigma1_posterior_uniform_prior_random_trend_model.R")

source("./simulation_study1/functions/b1_model/log_likelihood_random_trend_model.R")
...
for (h in 1 : 10) {
  ...
  for(nt in 0 : n.t){
    ...
    for(i in 2 : niter){
      ...
      # beta0
      beta0[nt * niter + i] <- beta0_power_posterior_random_trend_model(y, b1[, nt * niter + i - 1], sigma[ nt * niter + i - 1], A, tau_vector[nt + 1])
      
      # b0
      b1[, nt * niter + i] <-  b1_power_posterior_random_trend_model(y, beta0[nt * niter + i], sigma1[nt * niter + i -1], sigma[nt * niter + i -1], tau_vector[nt + 1])
      
      # sigma
      sigma[nt * niter + i] <- sigma_power_posterior_uniform_prior_random_trend_model(y, sigma[nt * niter + i - 1], beta0[nt * niter + i], b1[, nt * niter + i], a, b, tau_vector[nt + 1])
      
      # sigma0
      sigma1[nt * niter + i] <- sigma1_posterior_uniform_prior_random_trend_model(sigma1[nt * niter + i - 1], b1[, nt * niter + i], a, b)
  ...
  for(nt in 0 : n.t){
    for(i in 1 : niter_powpos){
      ...
      log_likelihood[ nt * niter_powpos + i] <- log_likelihood_random_trend_model(y, beta0[nt * niter + nburning + i], 
                                                                                      b1[, nt * niter + nburning + i], sigma[nt * niter + nburning + i], dim(y)[1], dim(y)[2])
...
app_evidence_power_posterior_random_trend_model_simulated_data <- app_evidence_power_posterior

save(app_evidence_power_posterior_random_trend_model_simulated_data, 
     file = "./simulation_study1/results/app_evidence_power_posterior_random_trend_model_simulated_data.RData")
load("./simulation_study1/results/app_evidence_power_posterior_random_trend_model_simulated_data.RData")
