...
# 2. Power posterior ------------------
# Iter
n.t <- 499 # number of different temperatures
c <- 5
#nburning <- 2000
nburning <- c(rep(50000, n.t + 1))
#niter_powpos <- 2000
# niter_powpos <- c(rep(200000, 5), rep(50000, 10), rep(30000, 5)) # probar con más
niter_powpos <- c(rep(200000, n.t + 1))
...
for (h in 1 : 10){
  ...
  write.table(app_evidence_power_posterior, 
              file = "./M1_sim_study2/app_evidence_power_posterior_M1.txt", 
              sep = ' ',
...
app_evidence_power_posterior_M3 <- app_evidence_power_posterior

# app_evidence_power_posterior_M1 <- c(-195.00734661383, -194.109998592322,
# -193.072561374816, -193.1277522443, -192.528125416176, -192.559004095964,
# -192.800695150378, -193.612146215456, -193.898439901952, -192.858463827781)

save(app_evidence_power_posterior_M3, 
     file = "./simulation_study/results/app_evidence_power_posterior_M3.RData")
