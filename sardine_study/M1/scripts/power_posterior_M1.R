# Power posterior sardinas uniform priors sigmas

# 0. functions-------------------

source("./sardine_study/M1/functions/power full conditionals/b0_power_posterior.R")
source("./sardine_study/M1/functions/power full conditionals/b1_power_posterior.R")
source("./sardine_study/M1/functions/power full conditionals/beta0_power_posterior.R")
source("./sardine_study/M1/functions/power full conditionals/sigma0_posterior_uniform_prior.R")
source("./sardine_study/M1/functions/power full conditionals/sigma1_posterior_uniform_prior.R")
source("./sardine_study/M1/functions/power full conditionals/sigma_power_posterior_uniform_prior.R")

source("./sardine_study/M1/functions/log_likelihood_mixed_linear_model.R")
...
# sigmas ~ U(a, b)    (también sigma0, sigma1)
a <- 0
b <- 5
...
for(nt in 0 : n.t){
  ...
  for(i in 2 : niter){
    ...
    # b1
    b1[, nt * niter + i] <-  b1_power_posterior(y, beta0[nt * niter + i], b0[, nt * niter + i], sigma1[nt * niter + i - 1], sigma[nt * niter + i -1], tau_vector[nt + 1])
    
    # sigma
    sigma[nt * niter + i] <- sigma_power_posterior_uniform_prior(y, sigma[nt * niter + i - 1], beta0[nt * niter + i], b0[, nt * niter + i], b1[, nt * niter + i], a, b, tau_vector[nt + 1])
...
for(nt in 0 : n.t){
  for(i in 1 : niter_powpos){
    ...
    log_likelihood[ nt * niter_powpos + i] <- log_likelihood_mixed_linear_model(y, beta0[nt * niter + nburning + i], b0[, nt * niter + nburning + i], b1[, nt * niter + nburning + i], sigma[nt * niter + nburning + i], dim(y)[1], dim(y)[2])
...
save(app_evidence_power_posterior_M1, 
     file = "./sardine_study/results/app_evidence_power_posterior_M1.RData")
