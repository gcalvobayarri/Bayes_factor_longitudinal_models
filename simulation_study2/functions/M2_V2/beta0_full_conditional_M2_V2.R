beta0_full_conditional_M2_V2 <- function(yy, tt, bb0, bb1, ssigma, AA, 
                                      sizess, tau = 1){
  # sizess: number of individual measurements
  # tau : temperature of the power posterior between 0 (prior) and 1 (posterior)
  err_matrix <- yy - ((t(t(bb0)) %*% rep(1, dim(yy)[2])) + (t(t(bb1)) %*% rep(1, dim(yy)[2])) * tt)
...
#beta0_full_conditional_M2_V2(Y, times, b0, b1, sigma, 10, n)
