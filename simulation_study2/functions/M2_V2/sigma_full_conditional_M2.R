sigma_full_conditional_M2 <- function(yy, tt, sigma_anterior, 
                                      beta00, bb1, aa, bb, sizess, tau = 1){
  ...
  sum_errors <- sum(
    (yy - (beta00 + (t(t(bb1)) %*% rep(1, dim(yy)[2])) * tt))^2, na.rm = T)
...
# sig <- .2
# sig <- sigma_full_conditional_M2(Y, times, sig, beta0, b1, 0, 10, n); print(sig)