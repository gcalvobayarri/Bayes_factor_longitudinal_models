b1_full_conditional_M2_V2 <- function(yy, tt, beta00, bb0, ssigma1, 
                                   ssigma, tau = 1){
  # tau : temperature of the power posterior between 0 (prior) and 1 (posterior)
  bb1 <- c()
  
  ty_matrix <- tt * (yy - (beta00 + (t(t(bb0)) %*% rep(1, dim(yy)[2]))))
  t2_matrix <- tt * tt
...
#b1_full_conditional_M2_V2(Y, times, beta0, b0, sigma1, sigma)
